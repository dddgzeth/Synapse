...
export async function POST(req: NextRequest) {
  ...
  const rawBase = body.apiSettings?.baseUrl?.trim() || process.env.ANTHROPIC_BASE_URL || "https://www.fucheers.top";
  const baseURL = rawBase.endsWith("/v1") ? rawBase : `${rawBase.replace(/\/$/, "")}/v1`;
  const apiKey = body.apiSettings?.apiKey?.trim() || process.env.ANTHROPIC_API_KEY || "";
  const model = body.apiSettings?.model?.trim() || process.env.ANTHROPIC_MODEL || "claude-sonnet-4-6";
